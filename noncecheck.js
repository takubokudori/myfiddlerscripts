...
*/
    ...
    static function OnDone(oSession: Session) {
        ...
        if(search(oSession.oRequest.headers.ToString(true,true,true),oSession.GetRequestBodyAsString())){
            oSession["ui-backcolor"] = "Orange";
            //oSession["ui-customcolumn"] = "change";
            //oSession["ui-hide"]="a";
        }
    }
    static function search(header: String,body: String){
        var arr=GetBodyArray(body);
        if(header.indexOf("nonce") != -1 || header.indexOf("security") != -1) return true;
        
        for(var i=0;i<arr.length;i++){
            if(arr[i][0].indexOf("nonce") != -1||arr[i][0].indexOf("security") != -1) return true;
        }
        return false;
    }
